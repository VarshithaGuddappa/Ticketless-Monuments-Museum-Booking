...
<?php
    ...
    if(isset($_POST['Usr-login']))
    {
      ...
      $u_pwd=($_POST['u_pwd']);//
     try{ $stmt=$mysqli->prepare("SELECT u_email, u_pwd, u_id FROM tms_user WHERE u_email=? and u_pwd=? ");
     //sql to log in user
      ...
      {//get user logs
        ...
        try {
        $log="insert into userLog(u_id, u_email, u_ip, u_city, u_country) values('$uid','$u_email','$ip','$city','$country')";
        $mysqli->query($log);
      } catch (mysqli_sql_exception $e) {
        error_log($e->__toString()); }
        if($log)
